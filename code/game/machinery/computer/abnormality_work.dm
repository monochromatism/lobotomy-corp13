...
/obj/machinery/computer/abnormality/proc/start_work(mob/living/carbon/human/user, work_type, training = FALSE)
	...
	while(total_boxes < work_time)
		if(!CheckStatus(user))
			break
		mod_work_speed = datum_reference.current.SpeedOverride(user, work_speed, mod_work_speed, work_type)
		if(do_after(user, temp_work_speed, src, IGNORE_HELD_ITEM))
			...
			mod_work_chance = datum_reference.current.ChanceOverride(user, work_chance, mod_work_chance, work_type)
